...
const PROGRESS_POST_INTERVAL_MS = 10_000;
const MP4_PROGRESS_POST_INTERVAL_MS = 5_000;
...
function LessonScreenpalEmbed({
  ...
  useEffect(() => {
    ...
    const id = window.setInterval(() => {
      ...
      const now = Date.now();
      const due =
        now - lastPostAtRef.current >= PROGRESS_POST_INTERVAL_MS ||
        maxWatchedRef.current >= durationCap * 0.9;
    ...
    return () => {
      ...
      // Flush latest position when leaving the page / unmounting
      postProgress(maxWatchedRef.current, durationCap);
    };
