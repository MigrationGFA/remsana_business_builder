...
export interface CacRegistration {
  id: string;
  registration_id?: string; // Backend may return this instead of id
  user_id: string;
...
export async function getCacRegistration(): Promise<CacRegistration | null> {
  ...
  try {
    ...
    // Normalize: backend may return registration_id instead of id
    return {
      ...data,
...
export async function createCacRegistration(registrationType: 'business_name' | 'company_incorporation'): Promise<CacRegistration> {
  ...
  // Normalize: backend returns registration_id, we need id for URL construction
  return {
    ...data,
    id: data.registration_id || data.id,
  } as CacRegistration;
...
/**
 * Step 1: Business/Company core info (names + objects)
 * Payload shape depends on registration_type:
 * - business_name: businessName, businessNameAlt, businessObjects[]
 * - company_incorporation: companyName, companyNameAlt, companyObjects (text), authorizedShareCapital
 */
...
export async function submitCacRegistration(regId: string): Promise<CacRegistration> {
  ...
  const { data } = await api.post<CacRegistration>(
    ...
    {}, // Backend expects a JSON body, even if empty
  );
